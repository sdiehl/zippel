...
use crate::modp::{add, mul, pow, Rng};
...
/// `deg / p` per step.
pub fn interpolate(f: &impl BlackBox, n: usize, p: u64, rng: &mut Rng) -> Option<ModPoly> {
    (0..3).find_map(|_| {
        let anchor: Vec<u64> = (0..n).map(|_| rng.nonzero(p)).collect();
        let mut h = first(f, &anchor, p, rng)?;
        for k in 1..n {
            h = lift(f, &h, k, &anchor, p, rng)?;
        }
        Some(h)
    })
}
...
fn first(f: &impl BlackBox, anchor: &[u64], p: u64, rng: &mut Rng) -> Option<ModPoly> {
...
fn lift(
    ...
    let (r, vals) = distinct_values(&skeleton, k, p, rng)?;
...
fn distinct_values(
    ...
    (0..3).find_map(|_| {
        let r: Vec<u64> = (0..k).map(|_| rng.nonzero(p)).collect();
